...
import { LineChartConfig } from '../../../models/ag-ui.models';
...
export class LineChartComponent extends BaseChartComponent implements OnChanges {
  @Input() declare config: LineChartConfig | undefined;
  ...
  private buildChart(): void {
    ...
    const baseOptions = this.themeService.getBaseChartOptions();
    ...
    const options: EChartsOption = {
      ...
      series: this.config.series.map((s, index) => ({
        ...
        smooth: this.config?.smooth ?? s.smooth ?? false,
        color: s.color || colors.primary[index % colors.primary.length],
        areaStyle: this.config?.showArea ? { opacity: 0.3 } : undefined,
        emphasis: {
          focus: 'series'
        },
        markPoint: {
  ...
  protected override getExportData(): unknown {
    return {
      xAxisData: this.config?.xAxisData,
